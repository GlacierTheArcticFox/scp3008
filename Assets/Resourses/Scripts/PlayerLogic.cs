...
public class PlayerLogic : MonoBehaviour {
    ...
    void Start() {
        //networkWorld = GameObject.Find("/NetworkWorld");
        //networkWorld.SetActive(true);
        world = GameObject.Find("/World");
    }

    [ServerRpc]
    void SpawnObjectServerRpc(ulong clientId, int objectId, string groupName){
        GameObject newObj = Instantiate(GameObject.Find("/World").GetComponent<WorldLogic>().prefabs[0], Vector3.zero, Quaternion.identity);
        newObj.GetComponent<NetworkObject>().Spawn();
        RemoveObjectClientRpc(objectId, groupName);
    }

    [ClientRpc]
    void RemoveObjectClientRpc(int objectId, string groupName) {
        Destroy(GameObject.Find(groupName).transform.GetChild(objectId).gameObject);
    }
            ...
            if (!isHit) {
                ...
                if (Physics.Raycast(ray, out hit, 2)) {
                    if (hit.collider.tag != "Static") {
                        isHit = true;
                        SpawnObjectServerRpc(NetworkManager.Singleton.LocalClientId, hit.collider.gameObject.transform.GetSiblingIndex(), hit.collider.gameObject.transform.parent.name);
                        /*hit.collider.gameObject.transform.GetSiblingIndex();
                        if (hit.collider.gameObject.transform.parent != GameObject.Find("MovedObjects").transform){
                            hit.collider.gameObject.GetComponent<NetworkObject>().Spawn();
                        ...
                        //hit.collider.gameObject.AddComponent<NetworkObject>();*/
                    }
                }
            } else {
